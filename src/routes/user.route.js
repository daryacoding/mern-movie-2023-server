...
router.post(
    ...
    .custom((value, { req }) => {
        if (value !== req.body.password) throw new Error("confirmPassword does not match")
        return true
    }),
    body("displayName")
...
router.post(
    ...
    .exists().withMessage("username is required")
    .withMessage("username minimum 8 charaters"),
    body("password")
    .exists().withMessage("password is required")
    .isLength({ min: 8}).withMessage("password minimum 8 characters"),
    requestHandler.validate,
...
router.put(
    ...
    .isLength({ min: 8}).withMessage("newPassword minimum 8 charaters"),
    body("confirmNewPassword")
    .exists().withMessage("confirmNewPassword is required")
    .isLength({ min: 8}).withMessage("confirmNewPassword minimum 8 charaters")
    .custom((value, { req }) => {
        if (value !== req.body.newPassword) throw new Error("confirmNewPassword does not match")
        return true
    }),
    requestHandler.validate,
